  ...
  return (
    <div className="mt-32">
      ...
      <p className="text-gray-500 text-sm" title={sprint?.description}>
        {sprint?.description?.substr(0, 140)}{sprint?.description && "..."}
      </p>
      <div className="mt-5 flexStart gap-3">
        <input
          className="w-40 mb-0 bg-gray-200 focus:w-52"
          type="text"
          placeholder="Search"
        />
      ...
      <div className={`flexBetween gap-5 mt-10`}>
        {sprint?.status.map((st) => (
          <div
            key={sprint.id}
            className=" bg-gray-200 min-h-[30rem] py-3 px-4 flex-1"
          >
            <div className="font-bold text-center mb-5">{st}</div>
            {/* {filterTasksByStatus(st).map((task) => (
              <div key={task?.id} className="p-3 bg-slate-800 font-normal">
                <div>{task?.title}</div>
                <div className="mt-5 capitalize">{task?.priority}</div>
                <div className="mt-5">
